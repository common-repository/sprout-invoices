...
function si_front_end_line_item( $item_data = array(), $position = 0, $prev_type = '', $has_children = false ) {
	print wp_kses_post( si_get_front_end_line_item( $item_data, $position, $prev_type, $has_children ) );
}

/**
 * Kses function for frontend line item html tags
 *
 * @return void
 */
function kses_si_front_end_line_item() {
	$allowed_html = array(
		'div'   => array(
			'class' => array(),
		),
...
function si_front_end_line_item_columns( $item_data = array(), $position = 0, $prev_type = '', $has_children = false ) {
	print wp_kses_post( si_get_front_end_line_item_columns( $item_data, $position, $prev_type, $has_children ) );
}
			...
			<div class="column column_<?php echo esc_attr( $column_slug ); ?>">
				...
				<?php if ( $prev_type !== $item_type || apply_filters( 'si_show_all_line_item_headers', true ) ) : ?>
					<?php if ( 'textarea' === $column_type ) : ?>
						<h3><?php printf( '%2$s <small>%1$s</small>', esc_html( number_format( (float) $position, 1, '.', '' ) ), wp_kses_post( $column['label'] ) ); ?></h3>
					<?php else : ?>
						<h3><?php echo wp_kses_post( $column['label'] ); ?></h3>
					<?php endif ?>
				<?php endif ?>
