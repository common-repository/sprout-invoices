 ...
 */
	function si_projects_select( $selected_id = 0, $client_id = 0, $blank = true, $el_id = 'doc_project' ) {
		...
		echo wp_kses( $out, SI_Settings_API::get_allowed_html() );
...
if ( ! function_exists( 'si_project_start_date' ) ) :
	function si_project_start_date( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_project_start_date', date_i18n( get_option( 'date_format' ), si_get_project_start_date( $id ) ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_project_end_date' ) ) :
	function si_project_end_date( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_project_end_date', date_i18n( get_option( 'date_format' ), si_get_project_end_date( $id ) ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_project_website' ) ) :
	function si_project_website( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_project_website', si_get_project_website( $id ), $id ) );
	}
endif;