...
if ( ! function_exists( 'si_invoice_status' ) ) :
	...
	function si_invoice_status( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_status', si_get_invoice_status( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_status_label' ) ) :
	...
	function si_invoice_status_label( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_status_label', si_get_invoice_status_label( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_issue_date' ) ) :
	...
	function si_invoice_issue_date( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_issue_date', date_i18n( get_option( 'date_format' ), si_get_invoice_issue_date( $id ) ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_due_date' ) ) :
	...
	function si_invoice_due_date( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_due_date', date_i18n( get_option( 'date_format' ), si_get_invoice_due_date( $id ) ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_expiration_date' ) ) :
	...
	function si_invoice_expiration_date( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_expiration_date', date_i18n( get_option( 'date_format' ), si_get_invoice_expiration_date( $id ) ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_id' ) ) :
	...
	function si_invoice_id( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_id', si_get_invoice_id( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_po_number' ) ) :
	...
	function si_invoice_po_number( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_po_number', si_get_invoice_po_number( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_client_id' ) ) :
	...
	function si_invoice_client_id( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_client_id', si_get_invoice_client_id( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_discount' ) ) :
	...
	function si_invoice_discount( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_discount', si_get_invoice_discount( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_tax' ) ) :
	...
	function si_invoice_tax( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_tax', si_get_invoice_tax( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_tax2' ) ) :
	...
	function si_invoice_tax2( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_tax2', si_get_invoice_tax2( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_payments_total' ) ) :
	...
	function si_invoice_payments_total( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_payments_total', sa_get_formatted_money( si_get_invoice_payments_total( $id ), $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_balance' ) ) :
	...
	function si_invoice_balance( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_balance', sa_get_formatted_money( si_get_invoice_balance( $id ), $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_calculated_total' ) ) :
	...
	function si_invoice_calculated_total( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_calculated_total', sa_get_formatted_money( si_get_invoice_calculated_total( $id ), $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_has_invoice_deposit' ) ) :
	...
	function si_has_invoice_deposit( $id = 0, $unfiltered = false ) {
		...
		$balence = si_get_number_format( si_get_invoice_payments_total( $id ) );
		if ( $balence >= $deposit ) {
			return false;
		}
...
if ( ! function_exists( 'si_invoice_deposit' ) ) :
	...
	function si_invoice_deposit( $id = 0, $unfiltered = false ) {
		...
		echo esc_html( apply_filters( 'si_invoice_deposit', si_get_invoice_deposit( $id, $unfiltered ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_total' ) ) :
	...
	function si_invoice_total( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_total', si_get_invoice_total( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_subtotal' ) ) :
	...
	function si_invoice_subtotal( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_subtotal', si_get_invoice_subtotal( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_sender_note' ) ) :
	...
	function si_invoice_sender_note( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_sender_note', si_get_invoice_sender_note( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_invoice_currency' ) ) :
	...
	function si_invoice_currency( $id = 0 ) {
		...
		echo esc_html( apply_filters( 'si_invoice_currency', si_get_invoice_currency( $id ), $id ) );
	}
endif;
...
if ( ! function_exists( 'si_payment_link' ) ) :
	...
	function si_payment_link( $id = 0, $type = '' ) {
		echo esc_url( si_get_payment_link( $id, $type ) );
	}
endif;
 ...
 *
	...
	function si_get_payment_url( $invoice_id = 0 ) {
		...
		if ( ! $invoice_id && isset( $_SERVER['REQUEST_URI'] ) ) {
			$invoice_id = url_to_postid( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
		}
...
function si_doc_history_records( $doc_id = 0, $filtered = true ) {
	...
	foreach ( $history as $item_id ) {
		...
		if ( get_post_type( $item_id ) === SI_Record::POST_TYPE ) {
			...
			if ( $filtered ) {
				if ( in_array( $record->get_type(), array( SI_Controller::PRIVATE_NOTES_TYPE, SI_Notifications::RECORD ), true ) ) {
					continue;
				}
			}
			...
			$returned_history[ $item_id ]['type']         = esc_html__( 'Payment', 'sprout-invoices' );
			...
			$returned_history[ $item_id ]['content'] .= '<b>' . esc_html__( 'Payment Total', 'sprout-invoices' ) . ':</b> ' . sa_get_formatted_money( $payment->get_amount(), $item_id );
		} else {
			if ( $filtered ) {
