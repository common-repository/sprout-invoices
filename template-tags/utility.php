...
if ( ! function_exists( 'si_format_address' ) ) :
	...
	function si_format_address( $address, $return = 'array', $separator = "\n" ) {
		...
		if ( ! empty( $address['street_2'] ) ) {
			$street_lines_2 = explode( "\n", $address['street_2'] );
			if ( ! empty( $street_lines_2 ) ) {
				foreach ( $street_lines_2 as $key => $street_2 ) {
					$lines[] = $street_2;
				}
			}
		}
...
function sa_currency_symbol() {
	echo esc_html( apply_filters( 'sa_currency_symbol', sa_get_currency_symbol() ) );
}
...
function si_number_format( $value = 1, $dec_point = '.', $thousands_sep = '', $fraction = 2 ) {
	echo esc_html( apply_filters( 'si_number_format', si_get_number_format( $value, $dec_point, $thousands_sep ), $value ) );
}
...
function si_purchase_link( $url = '', $campaign = 'free' ) {
	echo esc_url( si_get_purchase_link( $url, $campaign ) );
}
...
function sa_link( $url = '', $campaign = 'free' ) {
	echo esc_url( si_get_sa_link( $url, $campaign ) );
}
	...
	function si_localeconv( $doc_id = 0 ) {
		...
		if ( $locale !== false ) {
			...
			if ( isset( $localeconv['int_curr_symbol'] ) ) {
				switch ( trim( $localeconv['int_curr_symbol'] ) ) {
					case 'AUS':
					case 'GBP':
