 ...
 */
	function sa_form_fields( $fields, $context = 'contact', $wrap_class = '' ) {
		foreach ( $fields as $key => $data ) : ?>
				<?php if ( $data['type'] == 'heading' ) : ?>
				<legend class="legend form-heading <?php echo esc_attr( $context . $key . ' ' . $wrap_class ) ?>" ><?php echo esc_html( $data['label'] ) ?></legend>
			<?php elseif ( $data['type'] != 'checkbox' ) : ?>
				<div class="sa-control-group <?php echo esc_attr( $context . $key . ' ' . $wrap_class ) ?> <?php echo esc_attr( $data['type'] ) ?>">
					<span class="label_wrap"><?php sa_form_label( $key, $data, $context ); ?></span>
 ...
 */
	function sa_form_field( $key, $data, $category ) {
		echo wp_kses( apply_filters( 'sa_form_field', sa_get_form_field( $key, $data, $category ), $key, $data, $category ), SI_Settings_API::get_allowed_html() );
	}
endif;
...
?>
	...
	<?php if ( $data['type'] == 'textarea' ) : ?>
		<textarea name="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" id="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" rows="<?php echo isset( $data['rows'] ) ? esc_attr( $data['rows'] ) : 4; ?>" cols="<?php echo isset( $data['cols'] ) ? esc_attr( $data['cols'] ) : 40; ?>" <?php foreach ( $data['attributes'] as $attr => $attr_value ) { echo esc_attr( $attr ).'="'.esc_attr( $attr_value ).'" '; } ?> <?php if ( isset( $data['required'] ) && $data['required'] ) { echo 'required'; } ?> placeholder="<?php echo isset( $data['placeholder'] )?esc_attr( $data['placeholder'] ):esc_attr( $data['label'] ); ?>"><?php echo esc_html( $data['default'] ) ?></textarea>
	...
	<?php elseif ( $data['type'] == 'radios' ) : ?>
		...
		<?php endforeach; ?>
	<?php elseif ( $data['type'] == 'checkbox' ) : ?>
		<input type="checkbox" name="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" id="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" <?php checked( true, $data['default'] ); ?> value="<?php echo isset( $data['value'] ) ? esc_attr( $data['value'] ) : 'On'; ?>" <?php foreach ( $data['attributes'] as $attr => $attr_value ) { echo esc_attr( $attr ).'="'.esc_attr( $attr_value ).'" '; } ?> <?php if ( isset( $data['required'] ) && $data['required'] ) { echo 'required'; } ?>/>
	...
	<?php elseif ( $data['type'] == 'bypass' ) : ?>
		<?php echo wp_kses( $data['output'], SI_Settings_API::get_allowed_html() ); ?>
	<?php else : ?>
		<input type="<?php echo esc_attr( $data['type'] ) ?>" name="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" id="sa_<?php echo esc_attr( $category ) ?>_<?php echo esc_attr( $key ) ?>" class="text-input" value="<?php echo esc_attr( $data['default'] ) ?>" placeholder="<?php echo isset( $data['placeholder'] ) ? esc_attr( $data['placeholder'] ) : esc_attr( $data['label'] ); ?>" size="<?php echo isset( $data['size'] )? esc_attr( $data['size'] ) : 40; ?>" <?php foreach ( $data['attributes'] as $attr => $attr_value ) { echo esc_attr( $attr ).'="'.esc_attr( $attr_value ).'" '; } ?> <?php if ( isset( $data['required'] ) && $data['required'] ) { echo 'required'; } ?>/>
	<?php endif; ?>
 ...
 */
	function sa_form_field_classes( $data ) {
		...
		echo esc_attr( apply_filters( 'sa_form_field_classes', $classes, $data ) );
	}
endif;
 ...
 */
	function sa_form_label( $key, $data, $category ) {
		echo wp_kses( apply_filters( 'sa_form_label', sa_get_form_label( $key, $data, $category ), $key, $data, $category ), SI_Settings_API::get_allowed_html() );
	}
endif;
