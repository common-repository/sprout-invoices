...
class SI_Payment extends SI_Post_Type {
	...
	public function get_amount() {
		return (float) $this->get_post_meta( self::$meta_keys['amount'] );
	}
	...
	/**
	 * Check if a deposit payment has already been made.
	 *
	 * @since 20.4
	 * @param SI_Invoice $invoice SI_Invoice object.
	 * @return bool Returns true if a deposit payment has been made.
	 */
	public static function maybe_paid_deposit( $invoice ) {
		foreach ( $invoice->get_payments() as $payment_id ) {
			$payment      = SI_Payment::get_instance( $payment_id );
			$payment_type = $payment->get_type();
			if ( ! empty( $payment_type ) && in_array( 'deposit', $payment_type ) ) {
				return true;
			}
		}
		return false;
