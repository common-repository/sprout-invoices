<form id="si_checkout_review" action="<?php echo esc_attr( si_get_credit_card_checkout_form_action() ) ?>" method="post">
	<div class="checkout_block right_form clearfix">
		...
		<div class="paymentform-info">
			<h2 class="section_heading"><?php esc_html_e( 'Your Payment Information', 'sprout-invoices' ); ?></h2>
		</div>
	...
	<div class="checkout-controls clearfix">
		...
		<input type="hidden" name="<?php echo esc_attr( SI_Checkouts::CHECKOUT_ACTION ) ?>" value="<?php echo esc_attr( SI_Checkouts::REVIEW_PAGE ) ?>" />
		<input class="form-submit submit checkout_next_step" type="submit" value="<?php esc_html_e( 'Submit Order', 'sprout-invoices' ); ?>" name="si_checkout_button" />
	</div>
</form>
