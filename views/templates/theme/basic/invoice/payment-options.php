	...
	<div class="inner">
		...
		<?php if ( $time_left > 0 ) :  ?>
			...
			<?php if ( 1 === $days_left ) :  ?>
				...
				<?php
					printf(
					// translators: 1: opening strong tag, 2: invoice balance, 3: closing strong tag.
			...
			<?php else : ?>
				...
				<?php if ( si_has_invoice_deposit( get_the_id(), true ) ) : ?>
					<?php
						printf(
						// translators: 1: opening strong tag, 2: invoice deposit amount, 3: closing strong tag, 4: inovice balance, 5: days left to pay.
						esc_html__( 'Balance of %1$s%4$s%3$s Due in %1$s%5$s Days%3$s & Deposit of %1$s%2$s%3$s Due %1$sNow%3$s', 'sprout-invoices' ),
						'<strong>',
				...
				<?php else : ?>
					<?php
						printf(
						// translators: 1: opening strong tag, 2: invoice deposit amount, 3: closing strong tag, 4: days left to pay.
		...
		<?php else : ?>
			...
			<?php
				printf(
				// translators: 1: opening strong tag, 2: invoices balance, 3: closing strong tag.
