	...
	<div class="inner">
		...
		<div class="row title">
			...
			<?php if ( si_is_estimate_approved() ) : ?>
				<h2><?php esc_html_e( 'Approved Estimates', 'sprout-invoices' ) ?></h2>
			<?php else : ?>
				<h2><?php esc_html_e( 'Yet to be Approved Estimates', 'sprout-invoices' ) ?></h2>
			<?php endif; ?>
			...
			<?php foreach ( $totals as $slug => $items_total ) : ?>
				...
				<li id="line_<?php echo esc_attr( $slug ) ?>" class="row item">
					...
					<?php if ( isset( $items_total['helptip'] ) ) : ?>
						<h3 title="<?php echo esc_attr( $items_total['helptip'] ) ?>" class="helptip"><?php echo esc_html( $items_total['label'] ) ?></h3>
					<?php else : ?>
						<h3><?php echo esc_html( $items_total['label'] ) ?></h3>
					<?php endif ?>
					<?php $wp_kses_items_total = array( 'span' => array( 'class' => array() ) ) ?>
