...
<html>
	...
	<body id="estimate" <?php body_class( 'si_basic_theme' ); ?>>
			...
			<div class="inner">
				<div id="fromto_info" class="column">
					...
					<?php if ( si_get_estimate_client_id() ) : ?>
						...
						<h2 class="client_to"><?php esc_html_e( 'To', 'sprout-invoices' ); ?></h2>

						<div id="client_info">

							<h3><?php echo esc_html( get_the_title( si_get_estimate_client_id() ) ); ?></h3>
				...
				<div id="invoice_info" class="column">
					...
					<div class="invoice_info">
						<?php
							printf( '<span class="info">%1$s</span> %2$s',
					...
					<div class="invoice_info">
						<?php
							printf( '<span class="info">%1$s</span> %2$s',
							esc_html__( 'Expires on', 'sprout-invoices' ),
		...
		<section class="row" id="">
			<div class="inner">
				<div id="total_due" class="invoice_info">
					<?php
						printf( '<span class="info">%1$s</span> <span class="total">%2$s</span>',
						esc_html__( 'Total Estimated', 'sprout-invoices' ),
						esc_html( sa_get_formatted_money( si_get_estimate_total() ) )
						)
					?>
				</div>
			</div>
						...
						<?php if ( $last_updated = si_doc_last_updated() ) :  ?>
							...
							<?php if ( 2 > $days_since ) :  ?>
								<a class="open" href="#history">
									<?php
										printf(
										// translators: 1: days since estimate updated.
		...
		<section class="row" id="paybar">
			<div class="inner">
				...
				<?php if ( $time_left > 0 ) :  ?>
					...
					<?php
						printf(
						// translators: 1: opening strong tag, 2: estimate expires, 3: closing strong tag.
						esc_html__( 'Estimate expires in %1$s%2$s Days%3$s', 'sprout-invoices' ),
						'<strong>',
						esc_html( $days_left ),
						'</strong>'
						);
					...
					<?php if ( si_is_estimate_declined() ) : ?>
						<a class="decline_estimate" href="javascript:void(0)"><?php esc_html_e( 'Estimate Declined', 'sprout-invoices' ) ?></a>
					<?php else : ?>
					...
					<?php if ( si_is_estimate_approved() ) : ?>
						<a class="button accept_estimate" href="javascript:void(0)"><?php esc_html_e( 'Estimate Approved', 'sprout-invoices' ) ?></a>
					<?php else : ?>
				...
				<?php else : ?>
					<a class="button" href="javascript:void(0)"><?php esc_html_e( 'Estimate is Expired', 'sprout-invoices' ) ?></a>
				<?php endif ?>
		...
		<?php if ( apply_filters( 'si_show_estimate_history', true ) ) : ?>
			 <section class="panel closed" id="history">
				...
				<div class="inner">
					<h2><?php esc_html_e( 'Estimate History', 'sprout-invoices' ) ?></h2>
					<div class="history">
						<?php foreach ( si_doc_history_records() as $item_id => $data ) : ?>
							...
							<article class=" <?php echo esc_attr( $data['status_type'] ); ?>">
								<span class="posted">
									<?php
										// translators: 1: username of commentor
	...
	<?php printf( '<!-- Template Version v%s -->', esc_html( Sprout_Invoices::SI_VERSION ) ); ?>
