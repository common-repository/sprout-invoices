		...
		<div>
			<?php foreach ( $attachments as $media_id ) : ?>
				<?php
					...
					?>
				<span>
					<a href="<?php echo esc_attr( wp_get_attachment_url( $media_id ) ) ?>" download>
						<img src="<?php echo esc_url_raw( $icon ) ?>" title="<?php echo esc_attr( get_the_title( $media_id ) ) ?>" class="doc_attachment attachment_type_<?php echo esc_attr( $filetype['ext'] ) ?>" style="width: 40px; height: auto;">
					</a>
				</span>
			<?php endforeach ?>
		</div>
