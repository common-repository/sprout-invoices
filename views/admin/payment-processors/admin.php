							...
							<div id="col-left">
								<div id="sticky-wrapper" class="sticky-wrapper">
									<ul class="si-left-nav">
										...
										<a class="si-tab-item" href='#payment-processors' v-on:click="makeTabActive('payment-processors')" v-bind:class="{ active : isActiveTab('payment-processors') == true }"><?php esc_html_e( 'Payment Processors', 'sprout-invoices' ); ?></a>
									</ul>
								</div>
							</div>
										...
										<tbody>
											<?php foreach ( $all_processors as $class_name => $label ) : ?>
												<tr v-bind:class="{ activated : vm.<?php echo esc_attr( $class_name ); ?> == true }">
													<td v-bind:class="{ activating : isSaving == true }" id="<?php echo esc_attr( $class_name ); ?>"><?php echo wp_kses( $label, wp_kses_allowed_html( 'post' ) ); ?></td>
													<td class="status">
														<label for="pp-status">
															...
															<input v-if="Object.values(ccPP).includes( '<?php echo esc_attr( $label ); ?>' )" id="pp-status" type="checkbox" name="<?php echo esc_attr( $class_name ); ?>" v-model.lazy="vm.<?php echo esc_attr( $class_name ); ?>" v-on:change="activatePP">
