...
<div class="dashboard_widget inside">
	<div class="main">
		...
		<?php if ( ! empty( $estimates->posts ) ) : ?>
			...
			<ul>
				<?php foreach ( $estimates->posts as $estimate_id ) : ?>
					<li><a href="<?php echo esc_attr( get_edit_post_link( $estimate_id ) ) ?>"><?php echo esc_html( get_the_title( $estimate_id ) ) ?></a> &mdash; <?php echo esc_html( date_i18n( get_option( 'date_format' ), get_post_modified_time( 'U', false, $estimate_id ) ) ) ?></li>
				<?php endforeach ?>
			</ul>
		<?php else : ?>
			<p>
		...
		<?php if ( ! empty( $estimates->posts ) ) : ?>
			...
			<ul>
				<?php foreach ( $estimates->posts as $estimate_id ) : ?>
					<li><a href="<?php echo esc_attr( get_edit_post_link( $estimate_id ) ) ?>"><?php echo esc_html( get_the_title( $estimate_id ) ) ?></a> &mdash; <?php echo esc_html( date_i18n( get_option( 'date_format' ), get_post_time( 'U', false, $estimate_id ) ) ) ?></li>
				<?php endforeach ?>
			</ul>
		<?php else : ?>
			<p>
