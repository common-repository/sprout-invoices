<h3 class="dashboard_widget_title">
	<span><?php esc_html_e( 'Invoice Dashboard', 'sprout-invoices' ) ?></span>
</h3>
		...
		<?php if ( ! empty( $invoices->posts ) ) : ?>
			...
			<ul>
				<?php foreach ( $invoices->posts as $invoice_id ) : ?>
					<li><a href="<?php echo esc_attr( get_edit_post_link( $invoice_id ) ) ?>"><?php echo esc_html( get_the_title( $invoice_id ) ) ?></a> &mdash; <?php echo esc_html( date_i18n( get_option( 'date_format' ), get_post_modified_time( 'U', false, $invoice_id ) ) ) ?></li>
				<?php endforeach ?>
			</ul>
		<?php else : ?>
			<p>
		...
		<?php if ( ! empty( $invoices ) ) : ?>
			...
			<ul>
				<?php foreach ( $invoices as $invoice_id ) : ?>
					<li><a href="<?php echo esc_attr( get_edit_post_link( $invoice_id ) ) ?>"><?php echo esc_html( get_the_title( $invoice_id ) ) ?></a> &mdash; 
						<?php 
							printf( 
