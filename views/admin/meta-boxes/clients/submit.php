...
<div class="submitbox" id="submitpost">
	...
	<div id="minor-publishing">
		<p>
			<b><?php esc_html_e( 'Associated Users', 'sprout-invoices' ) ?></b> <span class="helptip" title="<?php esc_html_e( 'Clients can have multiple users associated. Each user will receive notifications.', 'sprout-invoices' ) ?>"></span>
			...
			<?php if ( ! empty( $associated_users ) ) : ?>
				<ul id="associated_users_list">
					<?php foreach ( $associated_users as $a_user_id ) : ?>
						...
						<li id="list_user_id-<?php echo (int) $a_user_id ?>">
							<?php 
								printf( '<a href="%1$s" class="si_tooltip" title="%s">%s</a>',
								esc_url( admin_url( 'user-edit.php?user_id=' . $a_user_id ) ),
								esc_attr( $u->user_email ), 
								esc_attr( $u->display_name )
								)
							?>  
							<a data-id="<?php echo (int) $a_user_id ?>" class="remove_user del_button">X</a>
							<?php do_action( 'client_associated_user_list', $a_user_id ) ?>
						</li>
					<?php endforeach ?>
				</ul>
			...
			<a href="#TB_inline?width=300&height=200&inlineId=user_creation_modal" id="user_creation_modal_tb_link" class="thickbox button" title="<?php esc_html_e( 'Create new user for this client', 'sprout-invoices' ) ?>"><?php esc_html_e( 'New User', 'sprout-invoices' ) ?></a>
		</p>
		<hr/>
				...
				<dl>
					<?php foreach ( $invoices as $invoice_id ) : ?>
						...
						<dd>
							<?php 
								printf( '<a href="%s">%s</a>',
								esc_attr( get_edit_post_link( $invoice_id ) ),
				...
				<dl>
					<?php foreach ( $estimates as $estimate_id ) : ?>
						...
						<dd>
							<?php 
								printf( '<a href="%s">%s</a>', 
								esc_attr( get_edit_post_link( $estimate_id ) ),
