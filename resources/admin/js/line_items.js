;(function( $, si, undefined ) {
	...
	si.lineItems.calculateTotal = function() {
		...
		if ( $tax > 0 ) {
			// adjust for tax
			$tax_total =  $sub_total * parseFloat( ( ( $tax ) / 100 ) ).toFixed(2);
		};

		var $tax2_total = 0;
		if ( $tax2 > 0 ) {
			// adjust for tax2
			$tax2_total =  $sub_total * parseFloat( ( ( $tax2 ) / 100 ) ).toFixed(2);
		};
		...
		if ( $discount > 0 ) {
			// adjust for discount
			$total = parseFloat( $total * ( ( 100 - $discount ) / 100 ) ).toFixed(2);
		};

		var $totalSpan      = $('#line_balance span'),
			$formattedTotal = parseFloat( $total ).toFixed(2);

		si.lineItems.totalUpdate( $totalSpan, $formattedTotal );

		$('#deposit input').attr('max',parseFloat( $total ).toFixed(2));

		$('ol.items_list').trigger( 'siCalculateTotal', [ $formattedTotal ] );
	}
	...
	si.lineItems.forceNumeric = function(element) {
		...
		// Only allow numbers 0 or higher.
		if ( element.hasClass('0_or_more') && element.numeric() && element.val() < 0 ) {
			element.val('');
		};
	}
	...
	si.lineItems.init = function() {
		...
		// Add item
		$(document).on('click','.item_add_type', function(e) {
		 ...
		 * @return {}
		 */
		$(document).on( 'click','.item_action.item_clone', function() {
		 ...
		 * @return {}
		 */
		$(document).on( 'click','.item_action.item_delete', function() {
		 ...
		 * @return {}
		 */
		$(document).on( 'keyup', '.totalled_input', function() {
			si.lineItems.calculateEachLineItemTotal();
			si.lineItems.calculateSubtotal();
			si.lineItems.calculateTotal();
