 ...
 * @since 20.7.0
 */
class SI_Field_Select extends SI_Field {
	/**
	 * Get Field Markup
	 *
	 * @since 20.7.0
	...
	public function get_field_markup() {
		?>
			...
			<label for="<?php echo esc_attr( $this->field_name ); ?>" class="si_input_label"><?php echo esc_html( $this->field_label ); ?></label>
				...
				>
				<?php
				foreach ( $this->field_options['options'] as $option => $value ) {
					?>
					<option value="<?php echo esc_attr( $option ); ?>" <?php echo esc_attr( $option === $this->field_options['value'] ? 'selected' : '' ); ?>><?php echo esc_html( $value ); ?></option>
					<?php
				}
				?>
				</select>
