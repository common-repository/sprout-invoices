...
abstract class SI_Payment_Processors extends SI_Controller {
	 ...
	 *
	 * @return  string $processor String of the active processor class name
	 */
	...
	/**
	 * Get available Credit Card Processors. In certain versions of the plugin, not all CC processors are included.
	 * This will return the available Credit Card Processors for a given install.
	 *
	 * @return array
	 */
	public static function available_cc_processors() {
		$available_cc_processors = array();
		$registered_processors   = array_keys( self::get_registered_processors() );
		$all_credit              = array(
			'SA_Stripe'           => 'Stripe',
		...
		foreach ( $all_credit as $class => $name ) {
			if ( in_array( $class, $registered_processors, true ) ) {
				$available_cc_processors[ $class ] = $name;
			}
		}
		return $available_cc_processors;
	}


	/**
	 * Register admin page within WordPress
	 *
	 * @param  array $admin_pages array of admin pages.
	...
	public static function register_admin_page( $admin_pages = array() ) {
		$admin_pages[ self::SETTINGS_PAGE ] = array(
			...
			'title'      => __( 'Payment Processors', 'sprout-invoices' ),
			'menu_title' => __( 'Payment Processors', 'sprout-invoices' ),
			'weight'     => 20,
			'section'    => 'payments',
	...
	/**
	 * Get Offsite Processor array.
	 *
	 * @since 20.7.0
	 *
	 * @param string $processor_type Type of processor to get.
	 *
	 * @return array
	 */
	public static function get_all_processors() {
		$offsite = self::get_registered_processors( 'offsite' );
		$credit  = self::get_registered_processors( 'credit' );

		return array_merge( $offsite, $credit );
	}

	/**
	 * Get Processor settings fields.
	 *
	 * This will return the settings fields for the payment processors that have settings
	 *
	 * @since 20.7.0
	 *
	 * @param string $class_name array of processors to filter.
	 *
	 * @return array $fields Array of settings fields for the payment processors.
	 */
	public static function get_processor_settings( $class_name ) {
		$pp_settings        = call_user_func( array( $class_name, 'register_settings' ) );
		$processor_settings = reset( $pp_settings );
		$processor_key      = key( $processor_settings );

		$fields = array();

		if ( isset( $processor_settings[ $processor_key ]['description'] ) ) {
			$fields[] = SI_Field_Factory::create(
				$processor_key . '_description',
				'', // No label for this field.
				'', // No description for this field.
				'paragraph',
				array(
					'attributes'    => array(
						'class' => 'si-pp-description',
					),
					'paragraph_text' => $processor_settings[ $processor_key ]['description'],
				)
			);
		}

		if ( isset( $processor_settings[ $processor_key ]['settings'] ) ) {
			foreach ( $processor_settings[ $processor_key ]['settings'] as $field_name => $field_configs ) {
	...
	/**
	 * Get Processor row options for the payment processors table.
	 *
	 * @since 20.7.0
	 *
	 * @param string $class_name array of processors to filter.
	 * @param string $label array of processors to filter.
	 *
	 * @return array $options Array of options for the payment processors table.
	 */
	public static function get_processor_row_options( $class_name, $label ) {
		$settings_button = '';

		// Conditonally render the settings link based on the processor.if there are settings to register.
		if ( method_exists( $class_name, 'register_settings' ) ) {
			$settings_button = SI_Field_Factory::create(
				...
				array(
					...
					'attributes'  => array(
						'class'      => 'si-settings-link button button-secondary',
						'v-if'       => 'vm.' . esc_attr( $class_name ),
		...
		$options = array(
			...
			'enabled'     => array(
				...
				'contents'   => SI_Field_Factory::create(
					...
					array(
						'vueifs'     => array(
							'v-if'   => '"Object.values(ccPP).includes( ' . esc_attr( $label ) . ' )"',
							'v-else' => '',
						),
						'attributes' => array(
							'id'           => 'pp-status',
							'name'         => esc_attr( $class_name ),
							'type'         => 'checkbox',
							'v-model.lazy' => 'vm.' . esc_attr( $class_name ),
							'v-on:change'  => 'activatePP',
						),
					)
				),
			),
		...
		// Stripe is deprecated and disabled. This will be removed in a future update.
		if ( 'SA_Stripe' === $class_name ) {
			$options['enabled']['contents']->field_options['attributes'] = array(
				'id'       => 'pp-status',
				'name'     => esc_attr( $class_name ),
				'type'     => 'checkbox',
				'disabled' => 'disabled',
	...
	/**
	 * Get Processor row upsell for the payment processors table.
	 *
	 * This is used to display an upsell message for the payment processors table when the user is on the free or freelancer versions of Sprout Invoices.
	 *
	 * @since 20.7.0
	 *
	 * @param string $class_name array of processors to filter.
	 ...
	 * @return array $options Array of options for the payment processors table.
	 */
	public static function get_processor_row_upsell( $class_name, $label ) {
		return array(
			'processor'   => array(
				'attributes' => array(
					'id' => esc_attr( $class_name ),
			...
			'actions'     => array(
				...
				'contents'   => self::get_upsell_button( $class_name, $label ),
			),
		);
	}

	/**
	 * Get Processor recommendations for setup wizard
	 *
	 * @since 20.7.0
	 *
	 * @return array
	 */
	public static function get_processor_recommendations() {
		return array(
			'SI_PayPal_Checkout'       => 'PayPal',
			'SI_Stripe_Checkout'       => 'Stripe Checkout',
			'SA_Square'                => 'Square',
			'SI_Woo_Payment_Processor' => 'WooCommerce Checkout',
		);
	}

	/**
	 * Get Upsell button for the payment processors table.
	 *
	 * This is used to redirect customers to the website to upgrade.
	 *
	 * @since 20.7.0
	 *
	 * @param string $class_name array of processors to filter.
	 ...
	 * @return array $options Array of options for the payment processors table.
	 */
	public static function get_upsell_button( $class_name, $label ) {
		return SI_Field_Factory::create(
			$class_name,
			$label,
			'',
			'button',
			array(
				'button_text' => __( 'Get Pro', 'sprout-invoices' ),
				'attributes'  => array(
					'class'  => 'si-upgrade-link button button-secondary',
					'href'   => esc_url( 'https://sproutinvoices.com/pricing' ),
					'target' => '_blank',
				),
			)
		);
	...
	/**
	 * Render the payment settings page to the admin menu
	 *
	 * @return void
	 */
	public static function render_settings_page() {
		...
		$all_offsite = self::get_all_processors( 'offsite' );
		$all_credit  = self::get_all_processors( 'credit' );
		...
		$settings['default_settings'] = array(
			...
			'settings' => array(
				self::MONEY_FORMAT_OPTION => array(
					'label'  => __( 'Currency Format', 'sprout-invoices' ),
					'option' => array(
						'type'        => 'input',
	...
	public static function manually_capture_payment() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['capture_payment_nonce'] ) ), self::AJAX_NONCE ) ) {
		...
		$payment_id = isset( $_REQUEST['payment_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['payment_id'] ) ) : 0;
		$payment    = SI_Payment::get_instance( $payment_id );
	...
	public static function manually_mark_complete() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['complete_payment_nonce'] ) ), self::AJAX_NONCE ) ) {
			self::ajax_fail( 'Not going to fall for it!' ); }
