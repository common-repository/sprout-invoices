	...
	public function process_payment_page( SI_Checkouts $checkout ) {
		...
		foreach ( array_keys( $fields ) as $key ) {
			if ( $key == 'cc_number' ) { // catch the cc_number so it can be sanatized
				if ( isset( $_POST['sa_credit_cc_number'] ) && strlen( sanitize_text_field( wp_unslash( $_POST['sa_credit_cc_number'] ) ) ) > 0 ) {
					$this->cc_cache['cc_number'] = preg_replace( '/\D+/', '', sanitize_text_field( wp_unslash( $_POST['sa_credit_cc_number'] ) ) );
				}
			} elseif ( isset( $_POST[ 'sa_credit_'.$key ] ) && strlen( sanitize_text_field( wp_unslash( $_POST[ 'sa_credit_'.$key ] ) ) ) > 0 ) {
