...
class SI_BACSs extends SI_Offsite_Processors {
	...
	public function process_payment( SI_Checkouts $checkout, SI_Invoice $invoice ) {
		$amount = ( isset( $_POST['sa_bacs_amount'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_bacs_amount'] ) ) : false;
		$number = ( isset( $_POST['sa_bacs_bac_number'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_bacs_bac_number'] ) ) : false;
		$date   = ( isset( $_POST['sa_bacs_mailed'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_bacs_mailed'] ) ) : false;
		$notes  = ( isset( $_POST['sa_bacs_notes'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_bacs_notes'] ) ) : '';

		if ( ! isset( $_POST['sa_bacs_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sa_bacs_nonce'] ) ), self::NONCE ) ) {
			return false;
		}
