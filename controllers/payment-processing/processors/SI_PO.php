...
class SI_PO extends SI_Offsite_Processors {
	...
	/**
	 * Register the payment processor in Sprout Invoices.
	 *
	 * @return void
	 */
	public static function register() {
		self::add_payment_processor( __CLASS__, __( 'PO Payment', 'sprout-invoices' ) );
	...
	public function process_payment( SI_Checkouts $checkout, SI_Invoice $invoice ) {
		$amount = ( isset( $_POST['sa_popayments_amount'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_popayments_amount'] ) ) : false ;
		$number = ( isset( $_POST['sa_popayments_check_number'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_popayments_check_number'] ) ) : false ;
		$notes = ( isset( $_POST['sa_popayment_notes'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_popayment_notes'] ) ) : '' ;

		if ( ! isset( $_POST['sa_popayments_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sa_popayments_nonce'] ) ), self::NONCE ) ) {
			return false;
		}
	...
	public static function show_attachments_meta_box( $post, $metabox ) {
		...
		if ( empty( $attachments ) ) :  ?>
			<?php printf( '<p>%s</p>', esc_html__( 'No payment attachments found.', 'sprout-invoices' ) ) ?>
		<?php else : ?>
			<style type="text/css">
			...
			<p id="payment_doc_attachment_thumbnails">
				<?php foreach ( $attachments as $media_id ) : ?>
					...
					<?php
						...
						?>
					<a href="<?php echo esc_url( get_edit_post_link( $media_id ) ) ?>" target="_blank" class="<?php echo esc_attr( $filetype['ext'] ) ?> payment_attachment" data-id="<?php echo esc_attr( $media_id ) ?>"><img src="<?php echo esc_html( $thumb_url ) ?>" /><br/><?php echo esc_html( get_the_title( $media_id ) ) ?></a>
				<?php endforeach ?>
			</p>
		<?php endif;
	}
	...
	public static function column_display( $item ) {
		...
		foreach ( $attachments as $media_id ) {
			...
			printf(
				'<a href="%s" target="_blank" class="%s payment_attachment" data-id="%s"><img src="%s" height="50px" width="auto"/></a>',
				esc_url( get_edit_post_link( $media_id ) ),
				esc_attr( $filetype['ext'] ),
				esc_attr( $media_id ),
				esc_html( $thumb_url ),
				esc_html( get_the_title( $media_id ) ) );
