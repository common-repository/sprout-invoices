...
class SI_Admin_Payment extends SI_Controller {
	...
	public static function create_admin_payment( $invoice_id = 0, $amount = '0.00', $number = '', $date = '', $notes = '' ) {
		...
		$payment_id = SI_Payment::new_payment(
			array(
				'payment_method' => self::get_payment_method() . ' - Added By: ' . $current_admin->get( 'user_nicename' ),
				'invoice'        => $invoice_id,
				'amount'         => $amount,
	...
	public static function show_payment_view( $post, $metabox ) {
		if ( $post->post_status == 'auto-draft' ) {
			printf( '<p>%s</p>', esc_html__( 'Save this invoice before adding any payments.', 'sprout-invoices' ) );
			return;
		}
	...
	public static function ajax_admin_payment() {
		...
		if ( isset( $_REQUEST['serialized_fields'] ) ) {
			foreach ( wp_unslash( $_REQUEST['serialized_fields'] ) as $key => $data ) {
				if ( strpos( $data['name'], '[]' ) !== false ) {
					...
					sanitize_text_field( $_REQUEST[ $data['name'] ] = $data['value'] );
				}
			}
		}
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sa_metabox_payments_nonce'] ) ), self::NONCE ) ) {
		...
		if ( get_post_type( sanitize_text_field( wp_unslash( $_REQUEST['sa_metabox_invoice_id'] ) ) ) != SI_Invoice::POST_TYPE ) {
		...
		self::create_admin_payment( sanitize_text_field( wp_unslash( $_REQUEST['sa_metabox_invoice_id'] ) ), $amount, $number, $date, $notes );
