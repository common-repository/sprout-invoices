...
class SI_Checks extends SI_Offsite_Processors {
	...
	/**
	 * Get the payment processor method.
	 *
	 * @return string
	 */
	...
	/**
	 * Get the payment processor slug.
	 *
	 * @return string
	 */
	...
	public static function register() {
		self::add_payment_processor( __CLASS__, __( 'Check', 'sprout-invoices' ) );
	}
	 ...
	 *
	 * @param  SI_Checkouts $checkout chekcout object.
	 * @return void
	 */
	public function payments_pane( SI_Checkouts $checkout ) {
		self::load_view(
			'templates/checkout/checks/form',
			array(
				'checkout'     => $checkout,
	...
	public function process_payment( SI_Checkouts $checkout, SI_Invoice $invoice ) {
		$amount = ( isset( $_POST['sa_checks_amount'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_checks_amount'] ) ) : false;
		$number = ( isset( $_POST['sa_checks_check_number'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_checks_check_number'] ) ) : false;
		$date   = ( isset( $_POST['sa_checks_mailed'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_checks_mailed'] ) ) : false;
		$notes  = ( isset( $_POST['sa_checks_notes'] ) ) ? sanitize_text_field( wp_unslash( $_POST['sa_checks_notes'] ) ) : '';

		if ( ! isset( $_POST['sa_checks_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash($_POST['sa_checks_nonce'] ) ), self::NONCE ) ) {
			return false;
		}
