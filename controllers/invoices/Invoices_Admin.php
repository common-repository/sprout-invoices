	...
	public static function column_display( $column_name, $id ) {
		...
		switch ( $column_name ) {
			case 'doc_link':
				...
				if ( $estimate_id ) {
					printf( '<a class="doc_link" title="%s" href="%s">%s</a>', esc_attr__( 'Invoice\'s Estimate', 'sprout-invoices' ), esc_url( get_edit_post_link( $estimate_id ) ), '<div class="dashicons icon-sproutapps-estimates"></div>' );
				}
			break;
			case 'dates':
			...
			case 'number':
				printf(
					...
					esc_attr__( 'View Invoice', 'sprout-invoices' ),
			...
			case 'total':
				...
				printf( '%s <span class="description">(%s %s)</span>', esc_html( sa_get_formatted_money( $total ) ), esc_html__( 'of', 'sprout-invoices' ), esc_html( sa_get_formatted_money( $invoice->get_total(), $invoice->get_id() ) ) );

				if ( 0.00 < $total ) {
					echo '<div class="row-actions">';
					printf( '<a class="payments_link" title="%s" href="%s&s=%s">%s</a>', esc_attr__( 'Review payments.', 'sprout-invoices' ), esc_url( get_admin_url( '','/edit.php?post_type=sa_invoice&page=sprout-apps/invoice_payments' ) ), esc_attr( $id ), esc_html__( 'Payments', 'sprout-invoices' ) );
				}
