...
class SI_Invoices_Edit extends SI_Invoices {
	...
	public static function save_meta_box_invoice_information( $post_id, $post, $callback_args, $invoice_id = null ) {
		...
		/**
		 * Verify that the post_type is a sa_invoice.
		...
		$due_m      = ( isset( $_POST['due_mm'] ) && $_POST['due_mm'] != '' ) ? sanitize_text_field( wp_unslash($_POST['due_mm'] ) ) : '' ;
		$due_j      = ( isset( $_POST['due_j'] ) && $_POST['due_j'] != '' ) ? sanitize_text_field( wp_unslash($_POST['due_j'] ) ) : '' ;
		$due_o      = ( isset( $_POST['due_o'] ) && $_POST['due_o'] != '' ) ? sanitize_text_field( wp_unslash($_POST['due_o'] ) ) : '' ;
		$invoice_id = ( isset( $_POST['invoice_id'] ) && $_POST['invoice_id'] != '' ) ? sanitize_text_field( wp_unslash( $_POST['invoice_id'] ) ) : '' ;
	...
	public static function show_invoice_send_view( $post, $metabox ) {
		if ( $post->post_status == 'auto-draft' ) {
			printf( '<p>%s</p>', esc_html__( 'Save this invoice before sending.', 'sprout-invoices' ) );
			return;
		}
		$invoice = SI_Invoice::get_instance( $post->ID );
	...
	public static function save_notes( $post_id, $post, $callback_args, $invoice_id = null ) {
		...
		$invoice_notes = ( isset( $_POST['invoice_notes'] ) && '' !== $_POST['invoice_notes'] ) ? wp_kses_post( wp_unslash( $_POST['invoice_notes'] ) ) : '';
		...
		if ( is_a( $client, 'SI_Client' ) ) {
			...
			foreach ( $client_users as $user_id ) {
				if ( ! is_wp_error( $user_id ) ) {
					$recipient_options .= sprintf(
						'<label class="clearfix"><input type="checkbox" name="sa_metabox_recipients[]" value="%1$s"> %2$s</label>',
						$user_id,
						esc_attr( SI_Notifications::get_user_email( $user_id ) )
					);
				}
			}
		}
