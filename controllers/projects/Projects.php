...
class SI_Projects extends SI_Controller {
	...
	public static function show_upgrade_notice() {
		if ( apply_filters( 'show_upgrade_messaging', true ) ) {
			printf( '<p><strong>Upgrade Available:</strong> Add time tracking and support the future of Sprout Invoices by <a href="%s">upgrading</a>.</p>', esc_url( si_get_purchase_link() ) );
		}
	}
	...
	public static function show_psp_meta_box( $post ) {
		printf( '<p class="description help_block"><a href="%s"><img src="%s" /></a><br/>%s</p>', 'https://sproutinvoices.com/sprout-invoices/integrations/', esc_url( SI_RESOURCES ) . 'admin/img/project-panorama-logo.png', esc_html__( 'SI now integrates with ', 'sprout-invoices' ) . '<a href="https://sproutinvoices.com/sprout-invoices/integrations/">' . esc_html__( 'Project Panorama', 'sprout-invoices' ) . '</a>.' );
	}
	...
	public static function save_meta_box_project_information( $post_id, $post, $callback_args, $estimate_id = null ) {
		...
		$start_date   = ( isset( $_POST['sa_metabox_start_date'] ) && $_POST['sa_metabox_start_date'] != '' ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_start_date'] ) ) : '' ;
		$end_date     = ( isset( $_POST['sa_metabox_end_date'] ) && $_POST['sa_metabox_end_date'] != '' ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_end_date'] ) ) : '' ;
		$default_rate = ( isset( $_POST['sa_metabox_default_rate'] ) && $_POST['sa_metabox_default_rate'] != '' ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_default_rate'] ) ) : '' ;
	...
	public static function form_fields( $project = 0, $required = false ) {
		...
		$description = ( $client_id ) ? printf(
				// translators: 1: Edit Link 2: Create New Client open tag 3: Closing tag.
				esc_html__( 'Edit %1$s, select another client or %2$screate a new client%3$s.', 'sprout-invoices' ),
				'<a href="' . esc_url( get_edit_post_link( $client_id ) ) . '">' . esc_html( get_the_title( $client_id ) ) . '</a>',
				'<a href="#TB_inline?width=600&height=450&inlineId=client_creation_modal" id="client_creation_modal_link" class="thickbox">',
				'</a>'
			)
			: printf(
				// translators: 1: opening a tag/client creation modal 2: closing a tag.
				esc_html__( 'Select an existing client or %1$s create a new client%2$s.', 'sprout-invoices' ),
				'<a href="#TB_inline?width=600&height=420&inlineId=client_creation_modal" id="client_creation_modal_link" class="thickbox">',
				'</a>'
			);
	...
	public static function add_projects_to_clients_admin() {
		echo esc_html( self::load_view( 'admin/meta-boxes/projects/client-submit', array(
	...
	public static function column_display( $column_name, $id ) {
		...
		switch ( $column_name ) {
			...
			case 'info':
				...
				echo '<p>';
				printf( '<b>%s</b>: ', esc_html__( 'Client', 'sprout-invoices' ) );
				if ( ! empty( $associated_clients ) ) {
					$clients_print = array();
					foreach ( $associated_clients as $client_id ) {
						$clients_print[] = sprintf( '<span class="associated_client"><a href="%s">%s</a></span>', esc_url( get_edit_post_link( $client_id ) ), esc_html( get_the_title( $client_id ) ) );
					}
				}
				if ( ! empty( $clients_print ) ) {
					echo wp_kses_post( implode( ', ', $clients_print ) );
				} else {
					echo esc_html__( 'No associated clients', 'sprout-invoices' );
				}
				echo '</p>';

				echo '<p>';
				printf( '<b>%s</b>: ', esc_html__( 'Site', 'sprout-invoices' ) );
				echo wp_kses( make_clickable( esc_url( $project->get_website() ) ), SI_Settings_Api::get_allowed_html() );
				echo '</p>';
				echo '</p>';
			...
			case 'invoices':
				...
				if ( ! empty( $split_invoices ) ) {
					...
					foreach ( $split_invoices as $invoice_id ) {
						printf( '<dt>%s</dt><dd><a href="%s">%s</a></dd>', esc_html( get_post_time( get_option( 'date_format' ) , false, $invoice_id ) ), esc_url( get_edit_post_link( $invoice_id ) ), esc_html( get_the_title( $invoice_id ) ) );
					}
					echo '</dl>';
				...
				printf( '<p><b>%s</b>: %s</p>', esc_html__( 'Total Invoiced ', 'sprout-invoices' ), esc_html( sa_get_formatted_money( $invoiced_total ) ) );

				if ( 0.00 < $outstanding_balance ) {

					printf( '<p><b>%s</b>: %s</p>', esc_html__( 'Outstanding Balance ', 'sprout-invoices' ), esc_html( sa_get_formatted_money( $outstanding_balance ) ) );
				}
			...
			case 'estimates':
				...
				if ( ! empty( $split_estimates ) ) {
					...
					foreach ( $split_estimates as $estimate_id ) {
						printf( '<dt>%s</dt><dd><a href="%s">%s</a></dd>', esc_html( get_post_time( get_option( 'date_format' ), false, $estimate_id ) ), esc_url( get_edit_post_link( $estimate_id ) ), esc_html( get_the_title( $estimate_id ) ) );
					}
					echo '</dl>';
					if ( count( $estimates ) > $split ) {
						printf(
							'%1$s of %2$s most recent shown%3$s',
							'<span class="description">...' . esc_html( $split ),
							'<a href="' . esc_url( get_edit_post_link( $id ) ) . '">' . esc_html( count( $estimates ) ) . '</a>',
							'</span>'
						);
					}
				} else {
	...
	public static function help_tabs() {
		...
		if ( $screen_post_type == SI_Project::POST_TYPE ) {
			...
			$screen->set_help_sidebar(
				sprintf( '<p><strong>%s</strong></p>', esc_html__( 'For more information:', 'sprout-invoices' ) ) .
				sprintf( '<p><a href="%s" class="button">%s</a></p>', 'https://sproutinvoices.com/support/knowledgebase/sprout-invoices/projects/', esc_html__( 'Documentation', 'sprout-invoices' ) ) .
				sprintf( '<p><a href="%s" class="button">%s</a></p>', esc_url( si_get_sa_link( 'https://sproutinvoices.com/support/' ) ) , esc_html__( 'Support', 'sprout-invoices' ) )
			);
		}
	}
