...
class SI_Clients extends SI_Controller {
	...
	public static function save_meta_box_client_information( $post_id, $post, $callback_args, $estimate_id = null ) {
		...
		$fax     = ( isset( $_POST['sa_metabox_fax'] ) && $_POST['sa_metabox_fax'] != '' ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_fax'] ) ) : '';

		$address = array(
			'street'      => isset( $_POST['sa_metabox_street'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_street'] ) ) : '',
			'street_2'    => isset( $_POST['sa_metabox_street_2'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_street_2'] ) ) : '',
		...
		if ( isset( $_POST['sa_metabox_email'] ) && $_POST['sa_metabox_email'] != '' ) {
			$user_args = array(
				'user_login'   => sanitize_text_field( wp_unslash( $_POST['sa_metabox_email'] ), 'sprout-invoices' ),
				'display_name' => isset( $_POST['sa_metabox_name'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_name'] ), 'sprout-invoices' ) : sanitize_text_field( wp_unslash( $_POST['sa_metabox_email'] ), 'sprout-invoices' ),
				'user_pass'    => wp_generate_password(), // random password
				'user_email'   => isset( $_POST['sa_metabox_email'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_email'] ), 'sprout-invoices' ) : '',
				'first_name'   => isset( $_POST['sa_metabox_first_name'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_first_name'] ), 'sprout-invoices' ) : '',
				'last_name'    => isset( $_POST['sa_metabox_last_name'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_last_name'] ), 'sprout-invoices' ) : '',
				'user_url'     => isset( $_POST['sa_metabox_website'] ) ? sanitize_text_field( wp_unslash( $_POST['sa_metabox_website'] ), 'sprout-invoices' ) : '',
			);
	...
	public static function show_client_history_view( $post, $metabox ) {
		if ( $post->post_status == 'auto-draft' ) {
			printf( '<p>%s</p>', esc_html__( 'No history available.', 'sprout-invoices' ) );
			return;
		}
		$client = SI_Client::get_instance( $post->ID );
	...
	public static function column_display( $column_name, $id ) {
		...
		switch ( $column_name ) {
			...
			case 'info':
				echo '<p>';
				$address = si_format_address( $client->get_address(), 'string' );
				...
				echo wp_kses( make_clickable( esc_url( $client->get_website() ) ), SI_Settings_API::get_allowed_html() );
				echo '</p>';

				$associated_users = $client->get_associated_users();
				echo '<p>';
				printf( '<b>%s</b>: ', esc_html__( 'Users', 'sprout-invoices' ) );
				...
				if ( ! empty( $users_print ) ) {
					echo wp_kses( implode( ', ', $users_print ), SI_Settings_API::get_allowed_html() );
				} else {
					echo esc_html__( 'No associated users', 'sprout-invoices' );
				}
				echo '</p>';
				...
				if ( ! empty( $split_invoices ) ) {
					...
					foreach ( $split_invoices as $invoice_id ) {
						printf(
							...
							esc_html( get_post_time( get_option( 'date_format' ), false, $invoice_id ) ),
							esc_url( get_edit_post_link( $invoice_id ) ),
							esc_html( get_the_title( $invoice_id ) )
						);
					}
					echo '</dl>';
					if ( count( $invoices ) > $split ) {
						printf(
							...
							'">',
							esc_html( count( $invoices ) ),
							'</a>'
						);
					}
				} else {
					printf( '<em>%s</em>', esc_html__( 'No invoices', 'sprout-invoices' ) );
				}
			break;
			...
			case 'estimates':
				...
				if ( ! empty( $split_estimates ) ) {
					...
					foreach ( $split_estimates as $estimate_id ) {
						printf(
							'<dt>%s</dt><dd><a href="%s">%s</a></dd>',
							esc_html( get_post_time( get_option( 'date_format' ), false, $estimate_id ) ),
							esc_url( get_edit_post_link( $estimate_id ) ),
							esc_html( get_the_title( $estimate_id ) )
						);
					}
					echo '</dl>';
					if ( count( $estimates ) > $split ) {
						printf(
							 ...
							 * 7: closing span tag
							 */
							esc_html__( '%1$s%2$s of %3$s%4$s%5$s%6$s%7$s most recent shown%8$s', 'sprout-invoices' ),
							'<span class="description">...',
							esc_html( $split ),
							'<a href="',
							esc_url( get_edit_post_link( $id ) ),
							'">',
							esc_html( count( $estimates ) ),
							'</a>',
							'</span>'
						);
					}
				} else {
					printf( '<em>%s</em>', esc_html__( 'No estimates', 'sprout-invoices' ) );
				}
			break;
	 ...
	 * @return string
	 */
	public static function user_column_display( $value, $column_name, $id ) {
		switch ( $column_name ) {
			case 'client':
	...
	public static function maybe_create_client() {
		...
		$nonce = sanitize_text_field( wp_unslash( $_REQUEST['sa_client_nonce'] ) );
		...
		if ( isset( $_REQUEST['sa_client_email'] ) && $_REQUEST['sa_client_email'] != '' ) {
			$user_args = array(
				'user_login'   => sanitize_text_field( wp_unslash( $_REQUEST['sa_client_email'] ) ),
				'display_name' => isset( $_REQUEST['sa_client_name'] )
					? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_name'] ) )
					: sanitize_text_field( wp_unslash( $_REQUEST['sa_client_email'] ) ),
				'user_pass'    => wp_generate_password(), // random password
				'user_email'   => isset( $_REQUEST['sa_client_email'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_email'] ) ) : '',
				...
				'user_url'     => isset( $_REQUEST['sa_client_website'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_website'] ) ) : '',
			);
			$user_id = self::create_user( $user_args );
		}
		...
		$address   = array(
			...
			'street_2'    => isset( $_REQUEST['sa_client_street_2'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_street_2'] ) ) : '',
		...
		$args      = array(
			...
			'phone'        => isset( $_REQUEST['sa_client_phone'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_phone'] ) ) : '',
			'fax'          => isset( $_REQUEST['sa_client_fax'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_fax'] ) ) : '',
			'website'      => isset( $_REQUEST['sa_client_website'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_client_website'] ) ) : '',
			'address'      => $address,
			'user_id'      => $user_id,
		);
	...
	/**
	 * Create WordPress user within Client Editor and attach the client profile to that user.
	 *
	 * @static
	 */
	public static function maybe_create_user() {
		...
		$nonce = sanitize_text_field( wp_unslash( $_REQUEST['sa_user_nonce'] ) );
		if ( ! wp_verify_nonce( $nonce, self::SUBMISSION_NONCE ) ) {
			self::ajax_fail( 'Not going to fall for it!' ); }
		...
		$client_id = isset( $_REQUEST['sa_user_client_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_user_client_id'] ) ) : '';
		...
		$user_args = array(
			'user_login'   => sanitize_text_field( wp_unslash( $_REQUEST['sa_user_email'] ) ),
			'display_name' => isset( $_REQUEST['sa_user_display_name'] )
				? sanitize_text_field( wp_unslash( $_REQUEST['sa_user_display_name'] ) )
				: sanitize_text_field( wp_unslash( $_REQUEST['sa_user_email'] ) ),
			'user_pass'    => wp_generate_password(), // random password
			'user_email'   => isset( $_REQUEST['sa_user_email'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['sa_user_email'] ) ) : '',
	...
	public static function submit_meta_box_view( $client_id = 0 ) {
		...
		print wp_kses( self::show_submit_meta_box( $client->get_post(), array() ), SI_Settings_API::get_allowed_html() );
		exit();
	}
