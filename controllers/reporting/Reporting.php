...
class SI_Reporting extends SI_Dashboard {
	...
	public static function get_chart_data() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['security'] ) ), self::AJAX_NONCE ) ) {
			self::ajax_fail( 'Not going to fall for it!' ); }

		$segment = ( isset( $_REQUEST['segment'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['segment'] ) ) : 'weeks' ;
		$span = ( isset( $_REQUEST['span'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['span'] ) ) : 'span';

		switch ( isset( $_REQUEST['data'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['data'] ) ) : '' ) {
