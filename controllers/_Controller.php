...
abstract class SI_Controller extends Sprout_Invoices {
	...
	public static function include_files() {
		$files = array(
			'/sprout-invoices-field.php',
			'/sprout-invoices-field-factory.php',
			'/sprout-invoices-field-tab.php',
			'/sprout-invoices-field-tabs.php',
			'/sprout-invoices-field-group.php',
			'/sprout-invoices-field-group-box.php',
			'/sprout-invoices-field-bypass.php',
			'/sprout-invoices-field-text.php',
			'/sprout-invoices-field-textarea.php',
			'/sprout-invoices-field-error.php',
			'/sprout-invoices-field-select.php',
			'/sprout-invoices-field-select-state.php',
			'/sprout-invoices-field-table.php',
			'/sprout-invoices-field-table-row.php',
			'/sprout-invoices-field-checkbox.php',
			'/sprout-invoices-field-button.php',
			'/sprout-invoices-field-paragraph.php',
	...
	protected static function clone_post( $post_id, $new_post_status = 'draft', $new_post_type = '' ) {
		...
		if ( isset( $post ) && $post != null ) {
			...
			// Update new post title with the correct id
			wp_update_post(
				array(
					'ID'         => $new_post_id,
			...
			if ( SI_Invoice::POST_TYPE === $post->post_type ) {
				...
				$days = apply_filters( 'si_default_due_in_days', 30 );
			...
			if ( SI_Estimate::POST_TYPE === $post->post_type ) {
				...
				// When cloning an estimate to invoice, no clean up necessary.
				if ( ! empty( $estimate ) ) {
					$days = apply_filters( 'si_default_expiration_in_days', 30 );
					$date = current_time( 'timestamp' ) + ( DAY_IN_SECONDS * $days );
					$estimate->set_expiration_date( $date );
				}
			}
		}
		// end
	...
	public static function maybe_clone_and_redirect() {
		if ( isset( $_GET['clone_si_post'] ) && isset( $_GET['post'] ) && sanitize_text_field( wp_unslash( $_GET['clone_si_post'] ) ) ) {
			$post_id = (int) $_GET['post'];
			if ( check_admin_referer( 'clone-si_post_'.$post_id, 'clone_si_post' ) ) {
				$new_post_type = ( isset( $_GET['post_type'] ) ) ? sanitize_text_field( wp_unslash( $_GET['post_type'] ) ) : '' ;
	...
	public static function get_standard_address_fields( $required = true, $user_id = 0 ) {
		...
		$fields['street_2'] = array(
			'weight'      => 60,
			'label'       => __( 'Street Address 2', 'sprout-invoices' ),
			'placeholder' => __( 'Apt, suite, unit, building, floor, etc.', 'sprout-invoices' ),
			'type'        => 'text',
			'rows'        => 2,
			'default'     => ( isset( $address['street_2'] ) ) ? $address['street_2'] : '',
			'required'    => false,
		);
		$fields['city'] = array(
			'weight' => 65,
	...
	/**
	 * Return an array of sanitized data.
	 * @param data array
	 * @return array
	 */

    public static function sanitize_serialized_field( $data ) {
