...
class SI_Internal_Records extends SI_Controller {
	...
	public static function maybe_purge_records() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST[ self::RECORD_PURGE_NONCE ] ) ), self::RECORD_PURGE_NONCE ) ) {
			return; }

		if ( isset( $_GET['purge_records'] ) ) {
			self::purge_records_display( sanitize_text_field( wp_unslash( $_GET['purge_records'] ) ) ); }
	}
	...
	public static function purge_records_display( $type = 0 ) {
		...
		foreach ( $records as $record_id ) {
			...
			document.getElementById("deletion_progress").innerHTML="<div style=\"width:' . esc_attr( $percent ) . ';background-color:#ddd;\">&nbsp;</div>";
			document.getElementById("deletion_information").innerHTML="' . sprintf( esc_html( '%o records(s) of %o deleted.', 'sprout-invoices' ), esc_html( $i ), esc_html( $total ) ) . '";
	...
	public static function maybe_delete_record() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['nonce'] ) ), SI_Controller::NONCE ) ) {
			wp_die( 'Not going to fall for it!' );
		}

		if ( current_user_can( 'manage_sprout_invoices_records' ) ) {
			$record_id = isset( $_REQUEST['record_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['record_id'] ) ) : null;
			wp_delete_post( $record_id, true );
	...
	public static function maybe_update_private_note() {
		...
		$record_id    = sanitize_text_field( wp_unslash( $_REQUEST['record_id'] ) );
		$private_note = isset( $_REQUEST['private_note'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['private_note'] ) ) : '';
		$record       = SI_Record::get_instance( $record_id );
	...
	<div class="wrap">
		<h2>
			<?php esc_html_e( 'Sprout Invoices Records', 'sprout-invoices' ); ?>
		</h2>
