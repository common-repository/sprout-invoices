...
class SI_Records_Table extends WP_List_Table {
	...
	function column_associate( $item ) {
		...
		if ( $associate_id > 1 ) {
			echo '<a href="'. esc_url( get_edit_post_link( $associate_id ) ) . '">' . esc_html( get_the_title( $associate_id ) ) . ' #' . esc_html( $associate_id ) . '</a>';
		}
	}
			...
			$tax_query = array(
					'tax_query' => array(
							array(
								'taxonomy' => SI_Record::TAXONOMY,
								'field' => 'id',
								'terms' => sanitize_text_field( wp_unslash( $_GET[SI_Record::TAXONOMY] ) )
							),
						),
				);
