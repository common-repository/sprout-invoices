...
class SI_Payments extends SI_Controller {
	...
	public static function maybe_void_payment() {
		...
		$nonce = sanitize_text_field( wp_unslash( $_REQUEST['void_payment_nonce'] ) );
		...
		if ( current_user_can( 'manage_sprout_invoices_payments' ) ) {
			...
			$data       = isset( $_REQUEST['notes'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['notes'] ) ) : '';
	...
	public static function display_table() {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				jQuery(".si_void_payment").on('click', function(event) {
					...
					$void_button.html("<?php esc_html_e( 'Working...', 'sprout-invoices' ); ?>");
					$.post( ajaxurl, { action: 'si_void_payment', payment_id: void_payment_id, notes: notes_form, void_payment_nonce: '<?php echo esc_attr( wp_create_nonce( SI_Controller::NONCE ) ); ?>' },
						function( data ) {
				...
				jQuery(".si_attempt_capture").on('click', function(event) {
					event.preventDefault();
					if( confirm( '<?php esc_html_e( 'Are you sure? This will force a capture attempt on this payment.', 'sprout-invoices' ); ?>' ) ) {
						var $capture_link = $( this ),
						capture_payment_id = $capture_link.attr( 'ref' );
						$capture_link.html('<?php esc_html_e( 'Working...', 'sprout-invoices' ); ?>');
						$.post( ajaxurl, { action: 'si_manually_capture_payment', payment_id: capture_payment_id, capture_payment_nonce: '<?php echo esc_attr( wp_create_nonce( SI_Payment_Processors::AJAX_NONCE ) ); ?>' },
							function( data ) {
				...
				jQuery(".si_mark_complete").on('click', function(event) {
					event.preventDefault();
					if( confirm( '<?php esc_html_e( 'Are you sure? This will mark the payment as complete.', 'sprout-invoices' ); ?>' ) ) {
						var $complete_link = $( this ),
						complete_payment_id = $complete_link.attr( 'ref' );
						$complete_link.html('<?php esc_html_e( 'Working...', 'sprout-invoices' ); ?>');
						$.post( ajaxurl, { action: 'si_mark_payment_complete', payment_id: complete_payment_id, complete_payment_nonce: '<?php echo esc_attr( wp_create_nonce( SI_Payment_Processors::AJAX_NONCE ) ); ?>' },
							function( data ) {
		...
		<div class="wrap">
			...
			<h2>
				<?php esc_html_e( 'Invoice Payments', 'sprout-invoices' ); ?>
			</h2>
