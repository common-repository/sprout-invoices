	...
	function column_total( $item ) {
		...
		if ( $invoice_id ) {
			...
			if ( is_a( $invoice, 'SI_Invoice' ) ) {
				echo '<strong>' . esc_html__( 'Payment Total', 'sprout-invoices' ) . ':</strong> ' . esc_html( sa_get_formatted_money( $payment->get_amount() ) ) . '<br/>';
				echo '<em>' . esc_html__( 'Invoice Balance', 'sprout-invoices' ) . ': ' . esc_html( sa_get_formatted_money( $invoice->get_balance(), $invoice->get_id() ) ) . ' </em><br/>';
				echo '<em>' . esc_html__( 'Invoice Total', 'sprout-invoices' ) . ': ' . esc_html( sa_get_formatted_money( $invoice->get_total(), $invoice->get_id() ) ) . '</em>';
			} else {
				esc_html_e( 'No invoice found', 'sprout-invoices' );
			}
		} else {
			esc_html_e( 'No invoice associated with this payment.', 'sprout-invoices' );
		}
	}
	...
	function prepare_items() {
		...
		$filter = ( isset( $_REQUEST['post_status'] ) ) ?
			...
			array(
				...
				SI_Payment::STATUS_PARTIAL,
				SI_Payment::STATUS_RECURRING,
				SI_Payment::STATUS_CANCELLED,
			);
