...
class SI_Notifications_Control extends SI_Controller {
	...
	public static function send_notification( $notification_name, $data = array(), $to = '', $from_email = null, $from_name = null, $html = null ) {
		...
		$bcc_name   = apply_filters( 'si_add_bcc_email', '' );
	...
	/**
	 * Check if notification was sent to a WordPress user.
	 * @todo This needs to be updated to check non wp users.
	 * @static
	 * @param string  $notification_name
	 * @param array   $data
	 * @param string  $to
	...
	public static function was_notification_sent( $notification_name, $data, $to, $notification_content = '' ) {
		...
		if ( is_array( $meta ) && in_array( self::get_hash( $data ), $meta ) ) {
			return true;
		}
		return false;
	}
	...
	public static function maybe_refresh_notification() {
		...
		if ( isset( $_GET['refresh-notification'] ) && is_numeric( $_GET['refresh-notification'] ) ) { // If dev than don't cache.
			...
			if ( get_post_type( sanitize_text_field( wp_unslash( $_GET['refresh-notification'] ) ) ) !== SI_Notification::POST_TYPE ) {
				return;
			}
	...
	/**
	 * Reset notification templates to the default values.
	 *
	 * @return void
	 */
	...
	public static function return_notification_html() {
		...
		if ( isset( $_GET['show-notification'] ) && sanitize_text_field( wp_unslash( $_GET['show-notification'] ) ) ) { // If dev than don't cache.
			...
			if ( self::html_notifications() ) {
				...
				print wp_kses_post( html_entity_decode( $si_notification->get_content() ) );
			} else {
				header( 'Content-Type: text/plain' );
				print wp_kses_post( $si_notification->get_content() );
			}
