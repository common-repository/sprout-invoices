...
class SI_Notifications extends SI_Notifications_Control {
	...
	public static function admin_payment_notification( SI_Payment $payment, $args = array() ) {
		...
		if ( $payment->get_status() === SI_Payment::STATUS_PENDING || $payment->get_status() === SI_Payment::STATUS_VOID ) {
			return;
		}
			...
			<table>
				<thead>
					<tr>
						...
						<th><?php esc_html_e( 'Quantity', 'sprout-invoices' ) ?></th>
						<th><?php esc_html_e( '% Adjustment', 'sprout-invoices' ) ?></th>
						<th><?php esc_html_e( 'Total', 'sprout-invoices' ) ?></th>
					</tr>
				</thead>
				<tbody>
							...
							<?php if ( is_numeric( $value['value'] ) && strpos( $value['data']['label'], __( 'Type', 'sprout-invoices' ) ) !== false ) : ?>
								...
								if ( ! is_wp_error( $term ) ) {
									esc_html_e( $term->name, 'sprout-invoices' );
										...
										?></p></dd>
							<?php else : ?>
								<dd><?php echo esc_html( apply_filters( 'the_content', $value['value'] ) ) ?></dd>
							<?php endif ?>
						<?php endif ?>
					<?php endforeach;
